...
public class PageTextureView extends GLSurfaceView {

    private final float suo = 180.0f / 320;//角度缩放比例
    private float lastX;//上次的触控位置Y坐标
    private float lastY;//上次的触控位置Y坐标

    private PageTextureRender mRenderer;

    public PageTextureView(Context context) {
        super(context);
        mRenderer = new PageTextureRender(context);   //创建场景渲染器
